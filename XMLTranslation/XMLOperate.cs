    ...
    class XMLOperate
    {
        ...
        public XMLObject XMLFiletoObject(String filePath, String fileName)
        {
            ...
            try
            {
                ...
                #region 标识信息
                XmlNodeList bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识");
                foreach (XmlNode bs_d in bs_data)
                {
                    ...
                    //其他多层数据
                    //数据集引用
                    XmlNodeList bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/数据集引用/CI_引用");
                    ...
                    //地理范围
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/地理范围/EX_地理坐标范围");
                    ...
                    //地理描述
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/地理描述/SI_地理描述");
                    xmlObject.BS_DLBSF = bs_other[0].SelectSingleNode("地理标识符").InnerText;

                    //时间范围
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/时间范围/EX_时间范围/范围/TM_时间段");
                    xmlObject.BS_QSSJ = bs_other[0].SelectSingleNode("起始时间").InnerText;
                    xmlObject.BS_ZZSJ = bs_other[0].SelectSingleNode("终止时间").InnerText;

                    //数据集联系信息
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/数据集联系信息/CI_负责单位");
                    ...
                    //数据集限制
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/数据集限制/MD_数据集限制");
                    ...
                    //数据集格式
                    bs_other = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/标识信息/MD_标识/数据集格式/MD_格式");
                ...
                #region 数据质量信息
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/数据质量信息");
                xmlObject.SJ_MS = bs_data[0].SelectSingleNode("概述").InnerText;
                xmlObject.SJ_SJZ = bs_data[0].SelectSingleNode("数据志").InnerText;
                #endregion

                #region 空间参照系统信息
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/空间参照系统信息/RS_参照系统/SC_大地坐标参照系统");
                ...
                #region 内容信息
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/内容信息/MD_内容描述");
                ...
                #region 分发信息
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/分发信息/MD_分发");
                xmlObject.FF_ZXLJ = bs_data[0].SelectSingleNode("数字传输选项").SelectSingleNode("在线连接").InnerText;
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/分发信息/MD_分发/分发者/MD_分发者/分发者联系信息/CI_负责单位");
                ...
                xmlObject.FF_ZZ3 = bs_data[0].SelectSingleNode("职责").NextSibling.NextSibling.InnerText;
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/分发信息/MD_分发/分发者/MD_分发者/分发者联系信息/CI_负责单位/联系信息/CI_联系");
                ...
                #region MD_元数据
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/MD_元数据");
                xmlObject.MD_RQ = bs_data[0].SelectSingleNode("日期").InnerText;
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/MD_元数据/联系/CI负责单位");
                ...
                bs_data = xmlDoc.SelectNodes("/福建省厦门市集美区基本农田永久划定成果元数据/MD_元数据/联系/CI_联系/联系信息");
        ...
        public bool ObjectToXMLFile(String filePath, XMLObject xmlObject)
        {
            ...
            sb.Append("\n<福建省厦门市集美区基本农田永久划定成果元数据 xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'>");
            sb.Append("\n\t<标识信息>");
            sb.Append("\n\t\t<MD_标识>");
            sb.Append("\n\t\t\t<数据集引用>");
            sb.Append("\n\t\t\t\t<CI_引用>");
